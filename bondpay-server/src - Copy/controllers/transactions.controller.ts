    ...
    try {
      ...
      if (incoming && Array.isArray(incoming)) {
        for (const item of incoming) {
          ...
          const totalBondValue = bonds.reduce((sum: number, b: any) => sum + b.value, 0);
          ...
          const bondIds = bonds.map((b: any) => b.bondId);
          bondIds.sort();
          ...
          const isTxValid = await CryptoService.verifySignature(txDataToVerify, transaction.senderSignature, senderRes.rows[0].public_key);
